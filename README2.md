# MySQL-MySQLdb
LEARN HOW TO USE PYTHON WITH MySQL
# MySQL-MySQLdb
LEARN HOW TO USE PYTHON WITH MySQL

零：
1.MySQLdb就是MySQL-python。
2.直接用pip不能找到，需要从网上找到exe版本来安装。
3.连接错误可能是由于MySQL没有启动
4.MySQL启动可以从我的电脑-管理-服务那里启动。
5.设置MySQL一些实务时，有时需要用管理员身份来运行cmd。
6.连接语句中是 passwd 而不是 password 。
...
mysqldb库的运用
一、
1.用help()命令后，再输入MySQLdb可以获得这个库的帮助。
2.import MySQLdb as msql
cxn = msql.connect(user='root',passwd='password')
cxn.query('CREATE DATABASE text')
cxn.query('DROP DATABASE text')
